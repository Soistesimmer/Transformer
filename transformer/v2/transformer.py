    ...
    with tf.name_scope("model"):
        if is_train:
            ...
            if params["enable_metrics_in_training"]:
                logits = metrics.MetricLayer(vocab_size)([logits, targets])  # 使用NCF，干了什么？
            logits = tf.keras.layers.Lambda(lambda x: x, name="logits",
                                            dtype=tf.float32)(logits)
            # tensorflow实例化方式之一，区别pytorch
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        with tf.name_scope("Transformer"):
            ...
            # padding设置为负无穷，使attention不会关注到padding的部分
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length, training):
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            if self.params["padded_decode"]:
                ...
                # 位置i的positional embedding与decoder input embedding相加
...
# 与encoder、decoder layer搭配，完成layernorm、dropout、残差操作
...
class EncoderStack(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for _ in range(params["num_hidden_layers"]):
            ...
            # 看出顺序：layernorm->self-attention->feed-forward->dropout->残差连接
            self.layers.append([
